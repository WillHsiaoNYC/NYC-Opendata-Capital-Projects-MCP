...
### How domain knowledge reaches agents

`src/od_cpd/primer.py` maintains named interpretation rules and an explicit
tool-to-rule map. The same rule text is delivered through:

- MCP initialization `instructions` (the complete primer).
- `dataset_info.domain_rules` (the complete primer as a list, for compatibility).
- Each tool's published description (only its applicable rules).
- Every successful MCP call's typed `interpretation_rules` list of `{id, text}`
  entries, in both `structuredContent` and serialized JSON text.

The tool descriptions and results are generated by the server, so guidance does
not depend on a client forwarding initialization instructions, reading repository
files, or calling `dataset_info` first. Tool-specific numeric evidence, scopes,
linked entities and caveats remain alongside the guidance. Rules are interpretation
instructions; deterministic tools enforce the calculations they implement, while
`run_sql` callers remain responsible for analytical correctness.

The stdio contract test verifies all four delivery surfaces, including a direct
schedule call before `dataset_info`. These checks establish server delivery and
behavior, not a guarantee that an LLM follows every rule in its final answer.
The project `AGENTS.md` documents the required workflow for future rule changes.
...
- `run_sql` — read-only SELECT against the DuckDB; `inline` / `csv` / `xlsx` export. Each export writes a fresh uniquely-named file under `exports/`. All three modes enforce the same query timeout (`RUN_SQL_TIMEOUT_SECONDS`, 30s) — a runaway query is interrupted, not left to hang. The read-only guard ignores string literals and comments (a literal `'%update%'` is fine), and its rejection message steers DESCRIBE/SHOW/PRAGMA callers to `describe_table`; xlsx stringifies LIST/STRUCT cells. The published description steers callers to the TYPED tables first and carries the shared grain rules (budget comparisons key on `(managing_agency, fms_id)`; sponsor-scoped budget sums use the `fms_sponsor` semi-join, never a value-bearing join). Every inline result echoes `latest_reporting_period`, and adds a `period_basis_note` when the query counts an all-history dimension (`fms_location` / `fms_sponsor` / `lifetime_budget_variance`) — those have no `reporting_period` column, so a raw count spans all periods, not one; truncated inline results carry a `truncation_note` steering to `output='csv'`
...
- **One forward link does not establish reciprocal 1:1 cardinality.** Detail tools
  describe one linked budget line or schedule without claiming exclusivity in the
  reverse direction. A budget without a linked schedule does not by itself indicate
  missing data. Links refer to the anchor's latest available link set, which need
  not be the current complete reporting snapshot.
- **Attributed funding is not an allocated share.** A shared funding line counts
  fully on each PID it funds. For a portfolio's distinct funding total, use
  `summary.line_budget_total`, which deduplicates the complete budget-line key;
  do not substitute a sum of PID-attributed budgets.
